...
def main():
    log.info("Tarea 2 (Process Dataflow) iniciada.")
    ...
    try:
        ...
        log.info(f"Procesando dataflow COMPLETO (Bronce+Plata): {dataflow_name}")

        # --- 2. Obtener Spark ---
        spark_session = utils.get_spark_session()
        
        # --- 3. Llamar al Orquestador ---
        # (El orquestador ya sabe cómo llamar a bronze.py y
        # luego a silver.py pasándole el initial_state)
        orchestrator.run_single_dataflow(spark_session, dataflow_config)
        
        log.info(f"Dataflow {dataflow_name} (COMPLETO) completado.")
